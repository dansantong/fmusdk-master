...
void TwinOpen(TwinModel* twin) {
	...
	if (connect(sockfd, (struct sockaddr *)&serv_addr, sizeof(SOCKADDR_IN)) != NOERROR) {
		...
		printf("InfluxDB connect() failed\n");
		exit(EXIT_FAILURE);
	}
	//后面写入数据时会用到
	twin->sockfd = sockfd;
...
void TwinInitialize(TwinModel* twin) {
	...
	if (err != 0) {
		...
		printf("could not open guid.txt\n");
		exit(EXIT_FAILURE);
	}
	fscanf(fp, "%d", &guid);
	fclose(fp);
...
void TwinSimulation(TwinModel* twin, char *header, char *body, char *result) {
	...
	//如果都为0，就不需要向InfluxDB写数据了
	if ((setNumber != 0) && (getNumber != 0)) {
		//0时刻的值写入
		...
		for (int i = 0; i < setNumber; i++) {
			...
			//将name中的,替换为.   空格也删掉   否则写入InfluxDB时会提示请求格式错误
			char finalName[1000];
			int k = 0;
			while (*name) {
				if (*name != ' ') {
			...
			switch (sv->typeSpec->type) {
				...
				case elm_Integer:
					...
					break;
				default:
					zlog_error(zc, "can not get value for type=%d\r\n", TwinGetVariableType(sv));
					/*printf("Simulation failed\n");
					exit(EXIT_FAILURE);*/
			}
			if (getNumber == 0) {
	...
	for (int i = 0; i < setNumber; i++) {
		...
		switch (sv->typeSpec->type) {
			...
			case elm_Integer:
				...
				break;
			default:
				zlog_error(zc, "can not set value for type=%d\r\n", TwinGetVariableType(sv));
				/*printf("Simulation failed\n");
				exit(EXIT_FAILURE);*/
		}
	}
...
double TwinGetVariableInit(ScalarVariable* sv, TwinModel* twin) {
	...
	switch (sv->typeSpec->type) {
		...
		case elm_Integer:
			...
			break;
		default:
			zlog_error(zc, "can not get init value for type=%d\r\n", TwinGetVariableType(sv));
			/*printf("Simulation failed\n");
			exit(EXIT_FAILURE);*/
			return 0;
	}
...
int main(int argc, char *argv[]) {
	...
	//上述分类用来写运行时的相关日志，而下面这个分类用来记录model properties相关的日志内容（因为直接输出到控制台上不好看）
	zc1 = zlog_get_category("property");
	if (!zc) {
		printf("get property category fail\n");
		zlog_fini();
		return -2;
	}
	...
	for (int k = 0; vars[k]; k++) {
		...
		zlog_info(zc1, "The %d-th variable: name is %s, valueReference is %d, valueType is %d, valueInit is %.16g, causality is %d.\r\n", k, TwinGetVariableName(sv), TwinGetVariableReference(sv), TwinGetVariableType(sv), TwinGetVariableInit(sv, &twin), TwinGetVariableCausality(sv));
	}
