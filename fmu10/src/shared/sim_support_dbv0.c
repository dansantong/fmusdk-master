...
void parseArguments(int argc, char *argv[], const char** fmuFileName, double* tEnd, double* h, int* loggingOn, char* csv_separator, int* setNumber, int* vr, double* value) {
	...
	if (argc > 6) {
		...
		if (sscanf(argv[6], "%d", setNumber) != 1) {
		...
		if (*setNumber != 0) {
			//下面要读输入的第7个参数
			int index = 7;
			//将后面跟上的valueReference和对应的值读进来
			for (int i = 0; i < *setNumber; i++) {
				if (sscanf(argv[index++], "%d", vr) != 1) {
					printf("error: The given parameter is uncorrect\n");
					exit(EXIT_FAILURE);
				}
				if (sscanf(argv[index++], "%lf", value) != 1) {
					printf("error: The given parameter is uncorrect\n");
					exit(EXIT_FAILURE);
				}
			}
