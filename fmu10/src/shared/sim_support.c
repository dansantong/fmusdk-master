...
void parseArguments(int argc, char *argv[], TwinModel* twin) {
    // parse command line arguments
    if (argc>1) {
        twin->fmuFileName = argv[1];
    }
    else {
	...
	if (argc > 2) {
		twin->ip_address = argv[2];
	}
	else {
		printf("error: no InfluxDB ip\n");
		printHelp(argv[0]);
		exit(EXIT_FAILURE);
	}
	if (argc > 3) {
		if (sscanf(argv[3], "%d", &(twin->port)) != 1) {
			printf("error: The given  InfluxDB port(%s) is not a number\n", argv[3]);
			exit(EXIT_FAILURE);
		}
	}
	if (argc > 4) {
		twin->database = argv[4];
	}
	else {
		printf("error: no InfluxDB database\n");
		printHelp(argv[0]);
		exit(EXIT_FAILURE);
	}
	if (argc >5) {
		twin->username = argv[5];
	}
	else {
		printf("error: no InfluxDB username\n");
		printHelp(argv[0]);
		exit(EXIT_FAILURE);
	}
	if (argc > 6) {
		twin->password = argv[6];
	}
	else {
		printf("error: no InfluxDB password\n");
		printHelp(argv[0]);
		exit(EXIT_FAILURE);
	}

    if (argc>7) {
        if (sscanf(argv[7],"%lf", &(twin->tEnd)) != 1) {
            printf("error: The given end time (%s) is not a number\n", argv[7]);
            exit(EXIT_FAILURE);
        }
    }
    if (argc>8) {
        if (sscanf(argv[8],"%lf", &(twin->h)) != 1) {
	...
	//解析用户要设置的初值
	if (argc > 9) {
		//setNumber是待设置初值的变量的个数
		if (sscanf(argv[9], "%d", &(twin->setNumber)) != 1) {
			printf("error: The given setNumber (%s) is not a number\n", argv[9]);
			exit(EXIT_FAILURE);
		}
		int index = 10;
		if (twin->setNumber != 0) {
			//根据setNumber分配数组空间
			twin->set_valueSeq = (int *)malloc((twin->setNumber) * sizeof(int));
			twin->set_value = (double *)malloc((twin->setNumber) * sizeof(double));
			//下面要读输入的第10个参数
			//将后面跟上的valueSequence和对应的值读进来
			for (int i = 0; i < twin->setNumber; i++) {
				if (sscanf(argv[index++], "%d", &(twin->set_valueSeq[i])) != 1) {
					printf("error: The given valueSequence is not a number\n");
					exit(EXIT_FAILURE);
				}
				if (sscanf(argv[index++], "%lf", &(twin->set_value[i])) != 1) {
					printf("error: The given init value is not a number\n");
					exit(EXIT_FAILURE);
				}
			}
		}
		//getNumber是想获取输出的变量的个数
		if (sscanf(argv[index], "%d", &(twin->getNumber)) != 1) {
			printf("error: The given getNumber (%s) is not a number\n", argv[index]);
			exit(EXIT_FAILURE);
		}
		index++;
		if (twin->getNumber != 0) {
			//根据getNumber分配数组空间
			twin->get_valueSeq = (int *)malloc((twin->getNumber) * sizeof(int));
			twin->output = (double *)malloc((twin->getNumber) * sizeof(double));
			for (int i = 0; i < twin->getNumber; i++) {
				if (sscanf(argv[index++], "%d", &(twin->get_valueSeq[i])) != 1) {
					printf("error: The given valueSequence is not a number\n");
					exit(EXIT_FAILURE);
				}
				twin->output[i] = 0;
...
void printHelp(const char* fmusim) {
    printf("command syntax: %s <model.fmu> <InfluxDB_ip> <port> <database> <username> <password> <tEnd> <tStep> <setNumber> <valueSequence> <setValue> <getNumber> <valueSequence> \n", fmusim);
    printf("   <model.fmu> .... path to FMU, relative to current dir or absolute\n");
	printf("   <InfluxDB_ip> .... IP address of InfluxDB for storing data\n");
	printf("   <port> .... The port that runs the InfluxDB HTTP service\n");
	printf("   <database> .... Name of the database used\n");
	printf("   <username> .... Username of the admin user\n");
	printf("   <password> .... Password of the admin user\n");
    printf("   <tEnd> ......... end  time of simulation, unit: second\n");
    printf("   <tStep> ............ step size of simulation, unit: second\n");
	printf("   <setNumber> ............ number of variables to set initial value\n");
    printf("   <valueReference> .... valueReference of variable to be set initial value\n");
	printf("   <setValue> .... init value to be set\n");
	printf("   <getNumber> ............ number of variables needed to get values\n");
    printf("   <valueSequence>............ valueSequence of variable whose value is to be get\n");
}
