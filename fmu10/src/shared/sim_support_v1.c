...
void parseArguments(int argc, char *argv[], TwinModel* twin) {
    ...
    if (argc>2) {
        if (sscanf(argv[2],"%lf", &(twin->tEnd)) != 1) {
            printf("error: The given end time (%s) is not a number\n", argv[2]);
            exit(EXIT_FAILURE);
        }
    }
    if (argc>3) {
        if (sscanf(argv[3],"%lf", &(twin->h)) != 1) {
	...
	if (argc > 4) {
		...
		if (sscanf(argv[4], "%d", &(twin->setNumber)) != 1) {
...
void printHelp(const char* fmusim) {
    printf("command syntax: %s <model.fmu> <tEnd> <tStep> <setNumber> <valueReference> <setValue> <getNumber> <valueSequence> \n", fmusim);
